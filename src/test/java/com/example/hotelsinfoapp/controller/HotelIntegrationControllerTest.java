...
class HotelIntegrationControllerTest extends BaseControllerTest {
    ...
    void getHotelByIdSuccessfulTest() throws Exception {
        ...
        mockMvc.perform(get(BASE_URI + "/" + receivedHotel.getId())
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpectAll(
                        status().isOk(),
                        content().contentType(MediaType.APPLICATION_JSON),
    ...
    void getHotelByIdDoesNotExistTest() throws Exception {
        ...
        mockMvc.perform(get(BASE_URI + "/" + nonExistingId)
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpectAll(
                        status().isNotFound(),
                        content().contentType(MediaType.APPLICATION_JSON),
                        ...
                        jsonPath("$.path", containsString(BASE_URI + "/" + nonExistingId)),
    ...
    @Test
    void addAmenitiesToHotelSuccessfulTest() {
        List<Hotel> hotelsInDb = hotelRepository.findAll();
        assertThat(hotelsInDb).isNotEmpty();
        ...
        assertThat(amenitiesInDb).isNotEmpty();

        Set<String> amenitiesNamesToAdd = new HashSet<>();
        do {
            Amenity receivedAmenity = amenitiesInDb.get(RANDOM.nextInt(amenitiesInDb.size()));
            amenitiesNamesToAdd.add(receivedAmenity.getName());
        } while (amenitiesNamesToAdd.size() != 3);
    }
